...
def get_max_file_bytes() -> int:
    env_val = os.environ.get("LINE_MCP_MAX_SOURCE_BYTES")
    if env_val:
        try:
...
def _read_file(path: Path, *, optional: bool, keep_data: bool, max_bytes: Optional[int] = None) -> _Observation:
...
def capture_snapshot(db_path: os.PathLike[str] | str, max_bytes: Optional[int] = None):
    ...
    for attempt in range(1, MAX_CAPTURE_ATTEMPTS + 1):
        ...
        try:
            database_first = _read_file(database_path, optional=False, keep_data=True, max_bytes=max_bytes)
            wal_first = _read_file(wal_path, optional=True, keep_data=True, max_bytes=max_bytes)
            wal_second = _read_file(wal_path, optional=True, keep_data=False, max_bytes=max_bytes)
            database_second = _read_file(database_path, optional=False, keep_data=False, max_bytes=max_bytes)
        except _SourceChanged:
            continue
